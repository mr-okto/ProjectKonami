...
class DbTest : public Test {
 public:
  DbTest() : Test(), test_uname_("John"), test_pwd_( "123456") {
    FakeDbConnector connector;
    session_ = new DbSession<FakeDbConnector>(std::move(std::make_unique<FakeDbConnector>(connector)));
  ...
  DbSession<FakeDbConnector>* session_;
};

TEST(DbSession, DbSession_Connect_Test) {
  FakeDbConnector connector;
  DbSession<FakeDbConnector> session(std::move(std::make_unique<FakeDbConnector>(connector)));
  auto result = session.connect("localhost", 5432, "db_user", "db_pass", "db_name");
  EXPECT_EQ(result, true);
}

TEST_F(DbTest, DbUserManager_CreateUser_Test) {
  UserManager<FakeDbConnector> user_manager(*session_);
...
TEST_F(DbTest, DbDialogueManager_GetDialogue_Test) {
  ...
  DialogueManager<FakeDbConnector> dialogue_manager(*session_);
...
TEST_F(DbTest, DbMessageManager_CreateMsg_Test) {
  ...
  MessageManager<FakeDbConnector> message_manager(*session_);
